...
def get_colors(color):
    if color == "uw58":
        ...
        return color_list.to(device).unsqueeze(1).unsqueeze(1).repeat(1, 10, 10, 1)
        ...
        for color in itertools.product(color, repeat=3):
            color_list[index] = torch.tensor(color) / 255
            index += 1
        return color_list.type(torch.float).to(device).unsqueeze(1).unsqueeze(1).repeat(1, 10, 10, 1)
